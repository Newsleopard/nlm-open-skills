...
| **支援工具** | Claude Code、Copilot、Cursor、Windsurf、Codex、OpenClaw | Claude Desktop、ChatGPT、Cursor、Lovable、GitHub Copilot、OpenAI Codex |

### MCP Server 設定

完整的安裝與設定教學請參考官方頁面：[電子豹 AI Agent 設定指南](https://newsleopard.com/ai-lab/ai-agent/)。

**連線資訊：**

- MCP 連線位址：`https://mcp.newsleopard.com/mcp`
- 認證方式：**OAuth 2.1**（首次連線時會引導登入並授權）

**支援的 AI 工具：** Claude（Desktop 版，需 Claude Pro/Team/Enterprise）、ChatGPT、Cursor、Lovable、GitHub Copilot、OpenAI Codex。

**快速設定步驟（以 Claude Desktop 為例）：**

1. 開啟 Settings → Connectors → 點擊「Add custom connector」
2. 名稱填入 `電子豹`，URL 填入 `https://mcp.newsleopard.com/mcp`
3. 點擊「Connect」，使用電子豹帳號登入並完成授權
4. 即可用自然語言下指令，例如「分析這個月的開信率」

MCP Server 提供完整工具集，涵蓋活動管理、報表分析、名單管理、範本操作、自動化控制等功能。
...
- [電子豹 MCP Server 設定指南](https://newsleopard.com/ai-lab/ai-agent/) — MCP Server 連線與授權教學
